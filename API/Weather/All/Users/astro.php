...
class Astro extends Db{
    ...
    public function get_astro($type, $user_id, $user_timezone, $api, $lat, $long){
        ...
        if($date_weather_forecast_inserted != $date_forecast && $type == 2){
            ...
            if($this->count() > 0){
                ...
                $this->query("UPDATE user_weather_astronomy SET DATE = ?, SUNRISE_HOUR = ?, SUNRISE_MINUTE = ?, SUNSET_HOUR = ?, SUNSET_MINUTE = ? , TIMEZONE = ? WHERE USER = ?");
            ...
            {
                $this->query("INSERT INTO user_weather_astronomy (USER,DATE,SUNRISE_HOUR,SUNRISE_MINUTE,SUNSET_HOUR,SUNSET_MINUTE,TIMEZONE) values (?,?,?,?,?,?,?)");
                  $this->bind(1,$user_id);
