...
class Hourly extends Db{
   public function get_hourly($type, $user_id, $place_id, $lat, $long){
        global $date;
        ...
        // Ensure we have an icon helper available
        ...
        if($date_weather_inserted <= $date_1 && $type == 2){
            ...
            if(isset($result) && (!isset($result->error) || $result->error == null)){
                        ...
                        if($this->count() > 0){
                            ...
                            $this->query("UPDATE Place_Weather_Hourly SET  
                                ...
                                SNOW_MM = ? , VISIBILITY_KM = ? , VISIBILITY_MPH = ? , SOURCE = ? , USER = ? WHERE PLACE = ?");

                        }else{

                            $this->query("INSERT INTO Place_Weather_Hourly 
                                (DATE, AQI, AQI_DESC, AQI_HEALTH, ICON, CONTENT, TEMP, TEMP_2, FEELS, FEELS_2,
                                DEW, DEW_2, HUMIDITY, PRESSURE, UVI, WIND_DEGREE, WIND_SPEED_KM, WIND_SPEED_MPH,
                                PRECIPITATION_PROB_PERCENT, PRECIPITATION_PROB_TYPE, PRECIPITATION_MM, SNOW_MM,
                                VISIBILITY_KM, VISIBILITY_MPH, SOURCE, USER, PLACE) values 
                                (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
