...
class FollowingUsers extends Db{
    public function following($user_id){
        ...
        $this->query("SELECT * from User_Follow  where USER_ID = ? AND USER_FOLLOWING_ID NOT LIKE ? ORDER BY (case when USER_FOLLOWING_ID = '1' then 1 when USER_FOLLOWING_ID = ? then 2 when USER_FOLLOWING_ID IN ($users_following) then 3 else 4 END), FIELD(USER_FOLLOWING_ID,$users_following), ID DESC");
        $this->bind(1, $user_id);
        $this->bind(2, $user_id);
        $this->bind(3, $user_id);
        ...
        foreach($all_users as $all_user){
            ...
            $users_id = $all_user['USER_FOLLOWING_ID'];
